...
class Create extends Action
{
    ...
    {
        $storeId= $this->backendModelSession->getStore()->getId();
        ...
        try {
            ...
            if ($quoteData->getEntityId() > 0) {
                ...
                if (implode(",", $pId) != '') {
                    ...
                    $formateShippingrate = $currencySymbol.number_format($shippingrate, 2, '.', '');
                    $formateTaxrate = $currencySymbol.number_format($taxrate, 2, '.', '');

                    $totals = $quoteData->getTotals(); //Total object
                    $grandtotal = $totals["grand_total"]->getValue(); //Grandtotal value
                    $formattedPrice = $currencySymbol.number_format($grandtotal, 2, '.', '');
                    if ($quoteData->getSubtotal()) {
                    ...
                    if ($this->_mailcartHelper->isPDFEnable()) {
                        $pdfContent = $this->createPdf($totals, $quoteData->getAllVisibleItems(), $quoteData->getCustomerFirstname() . ' ' . $quoteData->getCustomerLastname(), $quoteData->getCustomerEmail(), $quoteData, $shippingrate, $taxrate, $customerComment,$quoteData->getCouponCode(),$totalDiscount,$subtotal);
                    }
     ...
     * @return string
     */
    public function createPdf( $totals, $items, $customername, $customeremail, $quote, $shippingrate, $taxrate, $customerComment ,$coupon_code,$totalDiscount ,$subtotal)
    {
        try {
            $currencyCode =  $this->backendModelSession->getStore()->getCurrentCurrencyCode();
            $currency = $this->_currencyFactory->create()->load($currencyCode);
            ...
            $page->drawText("Quote : #" . $quote->getId(), $left + 350, $top, 'UTF-8');
            $page->drawText("Date  : " . date("d/m/Y", strtotime($quote->getCreatedAt())), $left + 350, $top - 20, 'UTF-8');
            $page->drawText($customeremail, $left + 350, $top - 40, 'UTF-8');
            ...
            foreach ($items as $item) {
                ...
                $custom_total = $item->getQty() * $item->getPrice();
                $page->drawText($currencySymbol.number_format($item->getPrice(), 2, '.', ''), $leftStart + 345, $topstart - 20, 'UTF-8');
                $page->drawText($currencySymbol.number_format($custom_total, 2, '.', ''), $leftStart + 450, $topstart - 20, 'UTF-8');
            ...
            $finaltotaltop = $topstart - 20;

            $page->drawLine(25, $finaltotaltop + 20, 570, $finaltotaltop + 20);

            $page->drawText("Sub Total : " .$currencySymbol.number_format($subtotal, 2, '.', ''), $leftStart + 375, $finaltotaltop, 'UTF-8');
            $finaltotaltop = $topstart - 40;

            $page->drawText("Shipping : " .$currencySymbol.number_format($shippingrate, 2, '.', ''), $leftStart + 380, $finaltotaltop, 'UTF-8');

            $finaltotaltop = $topstart - 60;

            $page->drawText("Coupon Code (" . $coupon_code . ") : -" .$currencySymbol.number_format($totalDiscount, 2, '.', ''), $leftStart + 320, $finaltotaltop, 'UTF-8');


            $finaltotaltop = $finaltotaltop - 20;

            $page->drawText("Tax Rate : " . $currencySymbol.number_format($taxrate, 2, '.', ''), $leftStart + 380, $finaltotaltop, 'UTF-8');

            $finaltotaltop = $finaltotaltop - 20;

            $page->drawText("Grand Total : " .$currencySymbol.number_format($totals["grand_total"]->getValue(), 2, '.', ''), $leftStart + 370, $finaltotaltop, 'UTF-8');
            $page->drawLine(25, $finaltotaltop - 20, 570, $finaltotaltop - 20);


            $commentTop = $finaltotaltop - 40;
            $customerCommentArray = $this->string->split($customerComment, 100);
            $page->drawText("Comment : ", $leftStart - 15, $commentTop, 'UTF-8');
            $commentTop -= 10;
            foreach ($customerCommentArray as $name) {
                $commentTop = $commentTop - 15;
                $page->drawText($name, $leftStart - 15, $commentTop, 'UTF-8');
            }
            $footertext = 30;
            // footer
    ...
    public function getLogo( $page )
    {
        ...
        $imagePath = '/logo/stores/1/logo_pdf.jpg';
