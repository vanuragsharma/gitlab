...
class BarcodeGenerator
{
    ...
    public function generateBarcodeImage($barcodeAsString, $type, $widthFactor = 2, $totalHeight = 30, $color = array(0, 0, 0))
    {
        $convertedType = $this->convertBarcodeType($type);
        $barcodeData = $this->getBarcodeData($barcodeAsString, $convertedType);
        ...
        if (function_exists('imagecreate')) {
            // GD library
            $imagick = false;
            $image = imagecreate($width, $height + $text_height);
            $colorBackground = imagecolorallocate($image, 255, 255, 255);
            imagecolortransparent($image, $colorBackground);
            $colorForeground = imagecolorallocate($image, $color[0], $color[1], $color[2]);
        }  else {
        ...
        foreach ($barcodeData['bars'] as $bar) {
            ...
            if ($bar['drawBar']) {
                ...
                // draw a vertical bar
                imagefilledrectangle($image, $positionHorizontal, $y, ($positionHorizontal + $bw) - 1, ($y + $bh),$colorForeground);
            }
        ...
        $font = 5;
        $x = (int)($width/4);
        imagestring($image, $font, $x, $height, $barcodeAsString, $colorForeground);

        return $image;
    }

    protected function convertBarcodeType($type)
    {
        $list = array(
            'Code128'=> self::TYPE_CODE_128,
            'Code25' => self::TYPE_STANDARD_2_5,
            'Code25interleaved' => self::TYPE_INTERLEAVED_2_5,
            'Code39' => self::TYPE_CODE_39,
            'Ean2' => self::TYPE_EAN_2,
            'Ean5' => self::TYPE_EAN_5,
            'Ean8' => self::TYPE_EAN_8,
            'Ean13' => self::TYPE_EAN_13,
            'Planet' => self::TYPE_PLANET,
            'Postnet' => self::TYPE_POSTNET,
            'Royalmail' => self::TYPE_RMS4CC,
            'Upca' => self::TYPE_UPC_A,
            'Upce' => self::TYPE_UPC_E
        );

        return (array_key_exists($type,$list)) ? $list[$type] : self::TYPE_CODE_128;
    }
