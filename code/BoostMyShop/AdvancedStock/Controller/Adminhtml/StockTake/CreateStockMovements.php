 ...
 */
class CreateStockMovements extends \BoostMyShop\AdvancedStock\Controller\Adminhtml\StockTake {
    ...
    public function execute(){
        ...
        try{
            ...
            if($id = filter_var($this->getRequest()->getParam('id'), FILTER_VALIDATE_INT)){
                ...
                if($stockTake->getId()){
                    $stockTake->createStockMovements();
                    if ($stockTake->getsta_per_location())
                        $stockTake->applyLocations();
                }
            }

            if (!isset($stockTake) || !$stockTake->getId()) {
                $this->messageManager->addErrorMessage(__('Unable to load stock take'));
