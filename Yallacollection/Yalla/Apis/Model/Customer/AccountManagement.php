...
use Yalla\Apis\Api\Customer\AccountManagementInterface;
use Magento\Customer\Api\AddressRepositoryInterface;
use Magento\Customer\Api\CustomerMetadataInterface;
use Yalla\Apis\Api\Customer\CustomerRepositoryInterface;
...
use Yalla\Apis\Model\Customer\Model\EmailNotificationInterface;
...
class AccountManagement implements AccountManagementInterface
{
    ...
    public function initiatePasswordReset($email, $template, $websiteId = null)
    {
        ...
        try {
            ...
            echo json_encode(['status'=>400, 'message'=> 'User with this email does not exist.']); exit;
        }
        ...
        try {
            switch ($template) {
                case AccountManagement::EMAIL_REMINDER:
                    $objectManager->create('Magento\Customer\Model\EmailNotificationInterface')->passwordReminder($customer);
                    break;
                case AccountManagement::EMAIL_RESET:
                    $objectManager->create('Magento\Customer\Model\EmailNotificationInterface')->passwordResetConfirmation($customer);
                    break;
                default:
            ...
            //return true;
            echo json_encode(['status'=>200, 'message'=> 'Password rest link has been sent, please check your inbox.']); exit;
        } catch (MailException $e) {
        ...
        //return false;
        echo json_encode(['status'=>500, 'message'=> 'There was an error processing the request. Please try again later.']); exit;
    }
     ...
     *
     * @param \Yalla\Apis\Api\Data\CustomerInterface $customer
	...
	if (!empty($postData)) {
		...
		if(isset($request['email']) && !empty($request['email'])){
			...
			$this->subscriberFactory->create()->subscribe($request['email']);
			return [['status' => 200, 'message' => 'Success']];
		}
	}
	...
	if (!empty($postData)) {
		$request = json_decode($postData, true);
		if(isset($request['email']) && !empty($request['email'])){
			$this->subscriberFactory->create()->loadByEmail($request['email'])->unsubscribe();
			return [['status' => 200, 'message' => 'Success']];
		}
	}
	return [['status' => 500, 'message' => 'Invalid email!']];
