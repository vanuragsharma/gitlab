...
class ApiData extends AbstractHelper
{
        ...
        if ($resultPage->getId()) {
            ...
            if ($otptype == "update") {
                if ($resultPage->getUpdateOtp() == $verifyCode) {
                    ...
                    $resultPage->save();
                    $customer = $this->loadCustomerById($this->getCustomerCollectionMobile($oldMobilenumer)->getId());
                    $customer->setCustomAttribute('mobile_number', $mobilenumber);
                    $this->customerRepository->save($customer);
                    if ($oldMobilenumer != "") {
                        $msg = "Mobile Updated from " . $oldMobilenumer . " to " . $mobilenumber . " Successfully";
                    } else {
                        $msg = "Mobile number has been updated successfully.";
                    }
                    $response['success'] = "true";
                    $response['successmsg'] = $msg;
                }
            }
        }
        return $response;
    ...
    public function curlApi($mobilenumber, $message)
    {
        try {
        	...
        	/*$enable = 1;
        	$senderId = 'Yallatoys';
            $sId = 'HXAP1693649939QA';
            $apiKey = 'Ab0e9d3cb98e16a1411dcdca513734faf';*/
        	if($enable == 1 && !empty($senderId) && !empty($sId) && !empty($apiKey)){
             ...
             if(strpos($mobilenumber, '+91') !== false)
             {
             	$postdata = "to=$mobilenumber&type=OTP&sender=$senderId&body=$message&template_id=$template_id";
             }else{
             	$postdata = "to=$mobilenumber&type=OTP&sender=$senderId&body=$message";
             }
	...
	$customer_mobile = $customer->getCustomAttribute('mobile_number');
	$mobile_number = '';
	if($customer_mobile){
